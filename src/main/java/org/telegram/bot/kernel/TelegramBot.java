...
public class TelegramBot {
    ...
    public TelegramBot(BotConfig config, ChatUpdatesBuilder chatUpdatesBuilder) {
        if (config == null) {
    ...
    public int init() throws NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
        ...
        final int loginResult = startKernelAuth(); // Perform login if necessary
    ...
    private void createKernelAuth() {
        final long start = System.currentTimeMillis();
        this.kernelAuth = new KernelAuth(this.apiState, config, this.kernelComm);
        BotLogger.info(LOGTAG, String.format("%s init in %d ms", this.kernelAuth.getClass().getName(), (start - System.currentTimeMillis()) * -1));
    }

    private int startKernelAuth() {
        final long start = System.currentTimeMillis();
        final int status = this.kernelAuth.start();
    ...
    private void createKernelComm() {
        final long start = System.currentTimeMillis();
        this.kernelComm = new KernelComm(this.apiState);
