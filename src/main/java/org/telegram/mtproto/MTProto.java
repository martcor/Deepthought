...
import static org.telegram.mtproto.secure.CryptoUtils.AES256IGEDecryptBig;
...
import static org.telegram.tl.StreamingUtils.intToBytes;
import static org.telegram.tl.StreamingUtils.longToBytes;
...
public class MTProto {
    ...
    private MTMessage decrypt(byte[] data, int offset, int len) throws IOException {
        ...
        byte[] sha1_a = SHA1(msgKey, substring(this.authKey, 8, 32));
        byte[] sha1_b = SHA1(substring(this.authKey, 40, 16), msgKey, substring(this.authKey, 56, 16));
        byte[] sha1_c = SHA1(substring(this.authKey, 72, 32), msgKey);
        byte[] sha1_d = SHA1(msgKey, substring(this.authKey, 104, 32));
